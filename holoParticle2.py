...
def process_for_training():
    datainfopath = './hologramPath/dataset/trainDataInfo2.csv'
    ...
    for i in range(len(holofilenames)):
        ...
        savedir_label = savedir + 'h'+str(z) + '/'
        os.makedirs(savedir_label, exist_ok = True)
        pts=[[x,y]]
        img_names = splitHolo_for_training(holo_im, pts, halfWin, savedir_label, imgIdx)
        for j in range(len(pts)):
